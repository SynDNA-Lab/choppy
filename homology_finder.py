...
def create_kmer_dictionary(sequence: SeqRecord, kmer_size: int) -> dict:
    ...
    # Forward strand
    for nucleotide in tqdm(range(1, len(sequence_str) + 1), desc="Processing forward strand"):
    ...
    for nucleotide in tqdm(range(1, len(sequence_rc_str) + 1), desc="Processing reverse strand"):
...
def update_kmer_dictionary(kmer_dict: dict, sequence: SeqRecord, kmer_size: int, offset: int):
    """
    ...
    for nucleotide in tqdm(range(1, len(sequence_str) + 1), desc="Updating forward strand"):
        if nucleotide + kmer_size <= len(sequence_larger):
            kmer = sequence_larger[nucleotide-1:nucleotide-1 + kmer_size]
    ...
    for nucleotide in tqdm(range(1, len(sequence_rc_str) + 1), desc="Updating reverse strand"):
        if nucleotide + kmer_size <= len(sequence_rc_larger):
            kmer = sequence_rc_larger[nucleotide-1:nucleotide-1 + kmer_size]
            if kmer in kmer_dict:
                kmer_dict[kmer].append(-nucleotide - offset)
...
def process_sequences(background_sequences: List[SeqRecord], query_sequences: List[SeqRecord], 
                     kmer_size: int) -> Dict[str, dict]:
    """
...
def find_non_homologous_regions(sequence_length: int, kmer_dict: dict, kmer_size: int, threshold: int) -> dict:
    """
    ...
    while i < len(non_homologous):
        ...
        while i + 1 < len(non_homologous) and non_homologous[i] + 1 == non_homologous[i + 1]:
            i += 1
...
def create_annotated_record(sequence: SeqRecord, regions: dict, output_format: str = 'genbank') -> SeqRecord:
    """
    ...
    for region_name, (start, end) in regions.items():
        feature = SeqFeature(
            ...
            type='misc_feature',
            qualifiers={'note': f'Non-homologous region {region_name}', 'label': f'homology_free'}
        )
...
def main():
    parser = argparse.ArgumentParser(description='Find non-homologous regions between sequences')
    parser.add_argument('query_sequence', help='Path to query sequence file(s) (GenBank or FASTA)')
    parser.add_argument('-b', '--background', nargs='+', required=True,
                      help='Path(s) to background sequence file(s) (GenBank or FASTA)')
    parser.add_argument('-k', '--kmer-size', type=int, default=20, 
                      help='Size of k-mers (default: 20)')
    parser.add_argument('-t', '--threshold', type=int, default=60,
                      help='Minimum size of non-homologous regions to report (default: 60)')
    parser.add_argument('-o', '--output', default='annotated_sequences.gb',
    ...
    kmer_dicts = process_sequences(background_sequences, query_sequences, args.kmer_size)
    ...
    # Write output file
    output_format = 'genbank' if args.output.endswith(('.gb', '.gbk', '.genbank')) else 'fasta'
    with open(args.output, 'w') as handle:
        SeqIO.write(output_records, handle, output_format)
    
    print(f"Processed {len(query_sequences)} query sequences against {len(background_sequences)} background sequences")
